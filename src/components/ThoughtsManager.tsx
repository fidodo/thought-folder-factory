...
const ThoughtsManager = () => {
  ...
  const ideas = thoughts.filter(
    (thought) =>
      thought.section === "ideas" &&
      (selectedFolder === null || thought.folder === selectedFolder),
  );
  const done = thoughts.filter(
    (thought) =>
      thought.section === "done" &&
      (selectedFolder === null || thought.folder === selectedFolder),
  );
  ...
  const deleteFolder = async (folderId: string) => {
    ...
    if (folderThoughts.length > 0) {
      ...
      if (
        !window.confirm(
          `This folder contains ${folderThoughts.length} thoughts. Delete anyway? (Thoughts will be moved out of the folder)`,
        )
      ) {
        return;
      }
    ...
    <div className="max-w-4xl mx-auto py-8">
      ...
      {totalThoughts === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center slide-in-up">
          ...
          <h1 className="text-3xl font-bold mb-4">Capture Your Thoughts</h1>
          <p className="text-muted-foreground mb-8 max-w-md">
            Start by adding your first thought below. Ideas can be organized,
            marked as done, and grouped into folders.
          </p>
        </div>
      )}
      ...
      {/* Main thought input */}
      <div
        className={`w-full max-w-2xl mx-auto ${totalThoughts === 0 ? "slide-in-up delay-200" : ""}`}
      >
        <ThoughtInput
          ...
          <div className="md:col-span-3 fade-in delay-100">
            ...
            <div className="flex items-center mb-5">
              ...
              <h2 className="font-medium text-lg">
                {selectedFolder ? `Thoughts in folder` : "All thoughts"}
              </h2>
            </div>
                      ...
                      >
                        {editingId === idea.id ? (
                          ...
                          <SortableItem
                            ...
                            folder={
                              idea.folder &&
                              folders.find((f) => f.id === idea.folder)?.name
                            }
